...
function QuestionCard({ question, questionIndex, totalQuestions, session, onNext, isStarted, onStart }) {
    ...
    return (
        <div className="bg-[#0a101f] border border-blue-900/30 rounded-3xl p-8 shadow-2xl space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1.5 ml-2">
                            {Array.from({ length: totalQuestions }).map((_, i) => (
                                <span
                                    key={i}
                                    className={`w-2 h-2 rounded-full transition-all duration-300 ${i < questionIndex
                                            ? 'bg-cyan-400'
                                            : i === questionIndex
                                                ? 'bg-blue-500'
                                                : 'bg-slate-700'
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {Object.entries(question.options).map(([letter, text]) => (
                            <button
                                ...
                                disabled={answered}
                            >
                                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${session?.correct_answer === letter
            ...
            {/* Result banner */}
            {session && (
                <div className={`rounded-xl px-4 py-3 border text-sm font-medium flex flex-col gap-2 ${session.is_correct
                                ...
                                {leaderboard.length > 0 ? (
                                    leaderboard.map((entry) => {
                                        ...
                                        return (
                                            <div
                                                key={entry.rank}
                                                className={`flex items-center justify-between rounded-xl px-3 py-2.5 transition-all duration-300 ${isMe
                                                        ? 'bg-blue-600/10 border border-blue-700/30'
                                                        : 'hover:bg-blue-900/20'
                                                    }`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${entry.rank === 1
